...
export const delete_home_post = async (req: TypedRequest, res: TypedResponse<APISuccessResponse<IHome> | APIErrorResponse>, next: TypedNextFn) => {
    try {
...
export const update_home_post = async (req: TypedRequest, res: TypedResponse<APISuccessResponse<IHome> | APIErrorResponse>, next: TypedNextFn) => {
    try {
        const id = req.params.id
        ...
        const home_response = await Home.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true });
